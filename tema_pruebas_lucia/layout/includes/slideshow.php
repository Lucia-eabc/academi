...
for ($s1 = 1; $s1 <= $numberofslides; $s1++) {
    $slideimage[] = theme_tema_pruebas_lucia_render_slideimg($s1, 'slide' . $s1 . 'image');
}
...
if ($countslideimage) {
    ...
    <div class="carousel-inner" role="listbox">
        ...
        <?php for ($s1 = 1; $s1 <= $numberofslides; $s1++) :
                $slidecaption = theme_tema_pruebas_lucia_get_setting('slide' . $s1 . 'caption', true);
                $slidedesc = theme_tema_pruebas_lucia_get_setting('slide' . $s1 . 'desc', 'format_html');

                $slideimg = theme_tema_pruebas_lucia_render_slideimg($s1, 'slide' . $s1 . 'image');
            if ($slideimg) {
                    $visableslide += 1;
