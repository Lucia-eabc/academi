 ...
 */
define("theme_tema_pruebas_lucia/theme", ["jquery"], function ($) {
  var img = $("nav#header").find(".avatar").find('img[src$="/u/f2"]'),
    src = img.attr("src");
  img.attr("src", src + "_white"),
    "true" === $("#header .navbar-nav button").attr("aria-expanded") &&
  ...
  0 == $.trim(foothtml).length &&
    $("footer#page-footer").addClass("empty-footer");
  var addhtml = $(".address-head").text();
  0 == $.trim(addhtml).length && $(".address-head").addClass("empty-address");
});
